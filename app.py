...
async def main():
    ...
    async def on_single_answer(c: CallbackQuery):
        if not engine.has_active(c.from_user.id):
            await c.answer()
            return
        letter = c.data.split(":", 1)[1]  # 'a'
        res = engine.check(c.from_user.id, letter)
        # очищаем мульти-буфер на всякий случай
        MULTI_BUF.pop(c.from_user.id, None)
        await handle_result(c.message, c.from_user.id, res)  # передаём user_id из callback
    ...
    async def on_multi_submit(c: CallbackQuery):
        ...
        answer_text = ",".join(sel)  # формат для engine._score_multi
